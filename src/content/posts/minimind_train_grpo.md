---
title: MiniMind GRPO 训练源码深度解析
published: 2026-02-27
description: "深入剖析 MiniMind 项目的 GRPO (Group Relative Policy Optimization) 算法实现。"
image: ""
tags: [LLM, Reinforcement Learning]
category: "Reinforcement Learning"
slug: minimind_train_grpo
draft: false
lang: "zh"
priority: 5
---

![GRPO算法流程图](./images/GRPO.png)
*图 1：GRPO 训练流程示意图*
...
$$
\mu = \frac{1}{K}\sum_{i=1}^K r_i,\quad
...
1. $\pi_\theta$ 为当前策略模型。
2. $\pi_{\text{ref}}$ 为参考模型（通常是 SFT 权重）。
3. $\beta$ 控制 KL 惩罚强度。

实现中常用 token 级 KL 近似：

$$
\Delta = \log \pi_{\text{ref}} - \log \pi_\theta,\quad
\mathrm{KL}_{\text{token}} \approx \exp(\Delta) - \Delta - 1
$$

### 3. 训练流程概览

1. 取一批 prompts。
2. 每个 prompt 生成 $K$ 条回复。
3. 计算奖励并组内标准化得到优势。
...
<details>
<summary><strong>代码：全局引用与环境初始化</strong></summary>
...
<details>
<summary><strong>代码：calculate_rewards</strong></summary>
...
<details>
<summary><strong>代码：grpo_train_epoch</strong></summary>
...
<details>
<summary><strong>代码：主入口</strong></summary>
